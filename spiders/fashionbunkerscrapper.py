...
class FashionbunkerScrapper(Spider_BaseClass):
    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(FashionbunkerScrapper, cls).from_crawler(crawler, *args, **kwargs)
        crawler.signals.connect(spider.spider_closed, signal=signals.spider_closed)
        return spider

    def GetProductUrls(self, homePageResponse):
        category_nodes = homePageResponse.xpath(
            "(//nav[@data-action='navigation']/ul/li[a[contains(text(),'NEW') or contains(text(),'Dresses') or "
            "contains(text(),'Tops') or contains(text(),'SALE') or contains(text(),'SHOP')]])[1]")
        for category_node in category_nodes:
            category_title = category_node.xpath('./a/text()').get().strip()
            print("top_category : ", category_title)
            sub_category_nodes = category_node.xpath("(.//div[@class='shop-by-brand-content']/ul/li[a/span[not("
                                                     "contains(text(),'All')) and contains(text(),'Dresses') or "
                                                     "contains(text(),'Jumpers') or contains(text(),'Jumpsuits')]])[1]")
    ...
    def listing(self, sub_category_link, category):
        ...
        product_nodes = categoryPageResponse.xpath("//a[@class='product-item-link']/@href").extract()
        ...
        try:
            next_page_url = categoryPageResponse.xpath("//a[@title='Next ']/@href").get()
    ...
    def GetPrice(self, response):
        orignalPrice = response.xpath(
            "//div[contains(@class,'product-info-price')]//span[contains(@id,'old')]/span/text()").get()
    ...
    def GetBrand(self, response):
        return response.xpath("//div[contains(@class,'brand-label')]/p/text()").get().strip()

    def GetDescription(self, response):
        return ' '.join(response.xpath("//div[contains(@class,'description')]//text()").extract()).strip()

    def GetSizes(self, response):
        sizes = []
        sizeList = response.xpath(
            "//span[@class='availability'][contains(text(),'InStock')]/following-sibling::span/span[@class='size']")
    ...
    def GetImageUrl(self, response):
        ...
        image_nodes = response.xpath(
            "//span[@class='alternate_image_urls']/span")
        for image in image_nodes:
            umage_url = image.xpath("./text()").get().strip()
