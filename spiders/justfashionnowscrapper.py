...
from FashionStores import *
store_url = sys.argv[4]


class JustfashionnowScrapper(Spider_BaseClass):
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(JustfashionnowScrapper, cls).from_crawler(crawler, *args, **kwargs)
        crawler.signals.connect(spider.spider_closed, signal=signals.spider_closed)
        return spider

    def GetProductUrls(self, response):
        category_json = '[{"id":1,"parent_id":0' + \
                        str(response.text).split('[{"id":1,"parent_id":0')[1].split('"men-collection"},"blog"')[
                            0] + '"men-collection"}}]'
        category_json = json.loads(category_json)
        for top_category in category_json:
            top_category_name = top_category['name']
            list = ['Best Sellers', 'Trending', 'Hoodies & Sweats', 'Tops', 'Bottoms', 'Shoes', 'ACC', 'Men']
            if Enumerable(list).where(lambda x: x in top_category_name).first_or_default():
                continue
            print('topo_cat_name :', top_category_name)
            category_nodes = top_category['children']
            for category_node in category_nodes:
                category_name = category_node['name']
                list2 = ["What's New", 'SPECIAL OFFER', 'BY PRICE', 'By Size', 'All Dresses']
        ...
        # url = "https://justfashionnow.com/products/denim-floral-casual-pants-3414714?variant=327829"
        # Spider_BaseClass.AllProductUrls.append(url)
        # Spider_BaseClass.ProductUrlsAndCategory[url] = 'new Bottom'
    ...
    def GetProducts(self, response):
        ...
        if (re.search('Sale', categoryAndName, re.IGNORECASE) or
            re.search('New', categoryAndName, re.IGNORECASE)) and not \
                re.search(r'\b((shirt(dress?)|jump(suit?)|dress|set|jeans|gown|suit|caftan)(s|es)?)\b', categoryAndName,
                          re.IGNORECASE):
    ...
    def GetImageUrl(self, response):
        ...
        for imageUrls in image_json:
            ...
            imageurl = "https://justfashionnow.com/image/" + image
